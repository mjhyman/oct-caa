function hist = measure_epvs_and_vessel_constant(hist,mrad,dpath,...
                                                histmatch_flag,plot_flag)
...
for ii = 1:length(hist)
    ...
    [epvs,ves] = dilate_meas_exclude_overlap(z_stain,mask,epvs,ves,se1,se2);
    
    %%% Add experimental + control to structure
    hist(ii).exp = epvs;
    hist(ii).ctl = ves;
    hist(ii).exp_mean = mean(epvs,'omitnan');
    hist(ii).ctl_mean = mean(ves,'omitnan');
...
if plot_flag
    for ii = 1:length(hist)
        %%% Plot stains
        % Load Stains
        sub_str = hist(ii).baseName;
        stain = hist(ii).stain;
        stain_matched = hist(ii).stain_matched;
        z_stain = hist(ii).z_stain;
        mask = logical(hist(ii).mask);
        % Set the background to 0
        stain(~mask) = 0;
        stain_matched(~mask) = 0;
        z_stain(~mask) = zmin;
        % Stain before processing
        figure; imagesc(stain,[0,max(stain(:))]);
        title({'Original Stain',sub_str},'Interpreter', 'none');
        clim([0,1]); c = colorbar; c.FontSize = 20;
        set(gca,'xtick',[]); set(gca,'ytick',[]);
        fname = strcat(sub_str,'_stain.png');
        fout = fullfile(dpath, fname);
        saveas(gcf,fout); close;
        % Histogram Matched
        figure; imagesc(stain_matched,[0,max(stain_matched(:))]);
        title({'Histogram Match',sub_str},'Interpreter', 'none')
        clim([0,1]); c = colorbar; c.FontSize = 20;
        set(gca,'xtick',[]); set(gca,'ytick',[]);
        fname = strcat(sub_str,'_stain_imhistmatch.png');
        fout = fullfile(dpath, fname);
        saveas(gcf,fout); close;
        % Z-score normalized
        figure; imagesc(z_stain,[0,max(z_stain(:))]);
        title({'Z-score Normalization',sub_str},'Interpreter', 'none')
        clim([zmin,zmax]); c = colorbar; c.FontSize = 20;
        set(gca,'xtick',[]); set(gca,'ytick',[]);
        fname = strcat(sub_str,'_zscore.png');
        fout = fullfile(dpath, fname);
        saveas(gcf,fout); close;
    end
end
