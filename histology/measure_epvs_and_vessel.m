function hist = measure_epvs_and_vessel(hist,radii,th)
...
for ii = 1:length(hist)
    ...
    for j=1:length(radii)
        ...
        [pstain,~] = dilate_meas(z_stain,mask,epvs,se1,se2);
        % Create map of pixels to exclude
        [ves,~] = dilate_meas(z_stain,mask,ves,se1,se2);
        
        %%% Add experimental + control to structure
        hist(ii).meas(j).exp = pstain;
        hist(ii).meas(j).ctl = ves;
        hist(ii).meas(j).exp_mean = mean(pstain,'omitnan');
        hist(ii).meas(j).ctl_mean = mean(ves,'omitnan');
    end
end
