...
%       - patch_len (uint): side length of control measurement square (pixels)
%       - patch_dist (uint): distance between control measurement squares (pixels)
%   OUTPUTS:
%       - hist (struct):
...
for ii = 1:length(hist)
    ...
    for j=1:length(radii)
        ...
        %%% Dilate EPVS mask & measure parenchyma
        [pstain,pstain_mask] = dilate_meas(z_stain,mask,epvs,se1,se2);
        
        %%% Measure control (uniform grid)
        % If patch_len undefined, then import control
        if ~isempty(patch_len)
            % Combine EPVS & dilated EPVS into logical matrix to exclude
            exclude_map = epvs | pstain_mask;
            % Take control measurements
            ctl = uniform_sample_ctl(z_stain, mask, exclude_map,...
                                    patch_len, patch_dist);
        else
            % Import vessel annotations
            ves = logical(hist(ii).ves);
            % Create map of pixels to exclude
            ctl = dilate_meas(z_stain,mask,ves,se1,se2);
        end
        
        %%% Add experimental + control to structure
        hist(ii).meas(j).exp = pstain;
        hist(ii).meas(j).ctl = ctl;
        hist(ii).meas(j).exp_mean = mean(pstain,'omitnan');
        hist(ii).meas(j).ctl_mean = mean(ctl,'omitnan');
    end
end
