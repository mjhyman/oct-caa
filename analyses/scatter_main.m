...
function scatter_main(subjects, fprops, offsets, standardize_flag)
...
for ii = 1:length(radii)
    ...
    for j = 1:length(subjects)
        ...
        for k = 1:length(regions)
            ...
            %%% percentage difference or offset removal
            [mus,ret] = relative_difference(standardize_flag, offsets,...
                                            sub, reg, mus, ret);
...
xlab = 'Distance (\mum)';
if standardize_flag == 2
    ylab_mus = '\mus (% \Delta)';
    ylab_ret = 'Retardance (% \Delta)';
else
    ylab_mus = '\mus (cm^-^1)';
    ylab_ret = 'Retardance (degrees)';
end
...
%% Percentage Difference or Median Removal

function [mus,ret] = relative_difference(standardize_flag, offsets,...
                                         sub, reg, mus, ret)
% RELATIVE_DIFFERENCE compute either % difference or remove offset
%   INPUTS
%       standardize_flag (int):
%                  0 : do not change measurement
%                  1 : use the absolute change from median
%                  2 : use the percentage change from median
%       offsets (boolean): subtract median white matter optical property
%       mus (boolean): scattering coefficient vector
%       ret (boolean): retardance vector
%   OUTPUTS
