...
plt_dir = '/projectnb/npbssmic/ns/CAA/figures/fig6_mus_ret_vs_SWP/';
...
%% Bin each subject for ease of scatterplot display
...
for ii = 1:length(subs)
    ...
    for j = 1:length(regs)
        ...
        [xy,se] = bin_swp(mus_pair, nbin);
        binned.(subs{ii}).(regs{j}).mus = xy;
        binned.(subs{ii}).(regs{j}).mus_se = se;
        % retardance
        [xy,se] = bin_swp(ret_pair, nbin);
        binned.(subs{ii}).(regs{j}).ret = xy;
        binned.(subs{ii}).(regs{j}).ret_se = se;
    end
    ...
    binned.(subs{ii}).comb.mus_se   = log_mus_se;
...
% scattering coefficient
[xy,se] = bin_swp(comb_log_mus, nbin);
binned.comb_mus = xy;
binned.comb_mus_se = se;
% retardance
[xy,se] = bin_swp(comb_log_ret, nbin);
binned.comb_ret = xy;
binned.comb_ret_se = se;
...
% Scattering
[xy,se] = bin_swp(front_log_mus, nbin);
binned.front_mus = xy;
binned.front_mus_se = se;
% Retardance
[xy,se] = bin_swp(front_log_ret, nbin);
binned.front_ret = xy;
binned.front_ret_se = se;

%%% Occipital
fprintf('Binning for occip\n')
% Scattering
[xy,se] = bin_swp(occip_log_mus, nbin);
binned.occip_mus = xy;
binned.occip_mus_se = se;
% Retardance
[xy,se] = bin_swp(occip_log_ret, nbin);
binned.occip_ret = xy;
...
xy = binned.comb_mus;
se = binned.comb_mus_se;
fname = strcat(substr,'comb_mus_swp');
swp_scatterplot(xy, xlab, ylab, tit, xlims, mus_lims, plt_dir, fname);
fname = strcat(substr,'comb_mus_swp_errorbar');
swp_scatterplot_errorbars(xy, se, xlab, ylab, tit, xlims, mus_lims, plt_dir, fname);
% retardance
ylab = 'retardance (degrees)';
tit = {'Frontal + Occipital', 'Retardance vs. EPVS log(SWP)'};
xy = binned.comb_ret;
se = binned.comb_ret_se;
fname = strcat(substr,'comb_ret_swp');
...
xy = binned.front_mus;
se = binned.front_mus_se;
fname = strcat(substr,'front_mus_swp');
swp_scatterplot(xy, xlab, ylab, tit, xlims, mus_lims, plt_dir, fname);
fname = strcat(substr,'front_mus_swp_errorbar');
swp_scatterplot_errorbars(xy, se, xlab, ylab, tit, xlims, mus_lims, plt_dir, fname);
% retardance
ylab = 'retardance (degrees)';
tit = {'Frontal', 'Retardance vs. EPVS log(SWP)'};
xy = binned.front_ret;
se = binned.front_ret_se;
fname = strcat(substr,'front_ret_swp');
swp_scatterplot(xy, xlab, ylab, tit, xlims, ret_lims, plt_dir, fname);
fname = strcat(substr,'front_ret_swp_errorbar');
swp_scatterplot_errorbars(xy, se, xlab, ylab, tit, xlims, ret_lims, plt_dir, fname);
...
xy = binned.occip_mus;
se = binned.occip_mus_se;
fname = strcat(substr,'occip_mus_swp');
swp_scatterplot(xy, xlab, ylab, tit, xlims, mus_lims, plt_dir, fname);
fname = strcat(substr,'occip_mus_swp_errorbar');
swp_scatterplot_errorbars(xy, se, xlab, ylab, tit, xlims, mus_lims, plt_dir, fname);
% retardance
ylab = 'retardance (degrees)';
tit = {'Occipital', 'Retardance vs. EPVS log(SWP)'};
xy = binned.occip_ret;
se = binned.occip_ret_se;
fname = strcat(substr,'occip_ret_swp');
swp_scatterplot(xy, xlab, ylab, tit, xlims, ret_lims, plt_dir, fname);
fname = strcat(substr,'occip_ret_swp_errorbar');
swp_scatterplot_errorbars(xy, se, xlab, ylab, tit, xlims, ret_lims, plt_dir, fname);
