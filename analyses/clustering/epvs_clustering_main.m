...
%%% Upper Percentile Threshold - this determines which EPVS to identify as
% the "seed" EPVS.
epvs_seed_th = 90;

%%% Minimum Threshold for Watershed (voxels)
% EPVS fewer than 5 voxels in radius will not be split. This is to prevent
% splitting disjoint small EPVS into fragments. The goal is to split
% EPVS branches from trunks
wmin = 5;

%%% Skeleton downsample
% factor to downsample the skeleton of the EPVS
skel_ds = 10;
...
% Update console
fprintf(['Running %s %s - %i percentile - %i watershed radius (voxels) - ' ...
    'skeleton downsample %i\n'], subject_name, region, epvs_seed_th,...
    wmin, skel_ds);
...
% Create filename to include parameters
% Create a filename to include parameters
filename = sprintf(['epvs_cluster_%s_%s_percentile_%i_WatershedRadius_%i_' ...
    'SkeletonDS_%i.mat'], subject_name, region, epvs_seed_th, wmin,skel_ds);

% Save
fprintf(['Saving %s %s - %i percentile - %i watershed radius (voxels) - ' ...
    'skeleton downsample %i\n'], subject_name, region, epvs_seed_th,...
    wmin, skel_ds);
fout = fullfile(cluster_dir, subject_name, region, filename);
