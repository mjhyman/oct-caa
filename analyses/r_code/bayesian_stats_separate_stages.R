...
make_formula_combined <- function(response, data) {
  n_subjects <- length(unique(data$subjectID))
  if (n_subjects >= 3) {
    as.formula(paste0(response, " ~ Groups * Stage * distance_f + (1 | subjectID)"))
  } else if (n_subjects == 2) {
    message("  2 subjects found – fitting fixed-effects model with subjectID covariate")
    as.formula(paste0(response, " ~ Groups * Stage * distance_f + subjectID"))
  } else {
    message("  1 subject found – fitting fixed-effects-only model")
    as.formula(paste0(response, " ~ Groups * Stage * distance_f"))
  }
}
...
for (stg in stages) {
  for (rgn in regions) {
    ...
    ret_sub  <- retardance_data %>%
      filter(Stage == stg, Region == rgn) %>%
      mutate(distance_f = factor(distance, levels = seq(40, 480, 40)))
    scat_sub <- scattering_data %>%
      filter(Stage == stg, Region == rgn) %>%
      mutate(distance_f = factor(distance, levels = seq(40, 480, 40)))

    out_dir <- file.path(output_filepath, base_name, paste0("stage", stg), label)
    dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

    fit_and_save_modality(ret_sub,  "retardance", stg, rgn, label, out_dir, base_name, fit_ctrl)
    fit_and_save_modality(scat_sub, "scattering",  stg, rgn, label, out_dir, base_name, fit_ctrl)

    message("  Finished: ", out_dir)
  }
}

######## 4) COMBINED MODEL ACROSS ALL STAGES ##################################
# message("\n===== Processing: combined all stages =====")
# 
# for (rgn in regions) {
#   
#   label <- paste0("combined_", rgn)
...
#     filter(Region == rgn) %>%
#     mutate(
#       distance_f = factor(distance, levels = seq(40, 480, 40)),
#       Stage      = factor(Stage, levels = c(0, 1, 2, 3),
...
#     )
#   
#   out_dir <- file.path(output_filepath, base_name, "combined", label)
#   dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
#   
#   fit_and_save_modality_combined(ret_sub,  "retardance", rgn, out_dir, base_name, fit_ctrl)
#   fit_and_save_modality_combined(scat_sub, "scattering",  rgn, out_dir, base_name, fit_ctrl)
#   
#   message("  Finished: ", out_dir)
# }