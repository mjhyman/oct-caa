...
%% Percentage Change Scatterplots (without error bars, diff y-axes)
...
for ii = 1:numel(f)
    ...
    for j = 1:numel(op)
        ...
        for k = 1:numel(regs)        
            ...
            tstr = sprintf('%s %s %s',f{ii}, op_title{j}, reg_title{k});
            title(tstr);
            % Subplot title
            subtitle('EPVS')
            % Figure Properties
            ...
            if save_flag
                pause(1)
                % Save as PDF
                saveas(gcf, fullfile(beta_dir, dirs.(f{ii}),...
                    sprintf('percentage_diff_%s_%s_%s_scatterplot_subfigs',...
                    f{ii}, op{j}, regs{k})), 'pdf');
                % Save as FIG
                saveas(gcf, fullfile(beta_dir, dirs.(f{ii}),...
                    sprintf('percentage_diff_%s_%s_%s_scatterplot_subfigs',...
                            f{ii}, op{j}, regs{k})), 'fig');
                % Save as PNF
                saveas(gcf, fullfile(beta_dir, dirs.(f{ii}),...
                    sprintf('percentage_diff_%s_%s_%s_scatterplot_subfigs',...
