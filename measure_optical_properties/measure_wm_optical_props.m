...
[mus_mat,ret_mat,ori_mean,ori_std] =...
    parenchyma_optical_props(seg_wm,mus,ret,ori,se);
% Add to struct
parench.(sub).(loc).ves.pmus = mus_mat;
parench.(sub).(loc).ves.pret = ret_mat;
parench.(sub).(loc).ves.pori_mean = ori_mean;
parench.(sub).(loc).ves.pori_std = ori_std;
...
%%% Measure optical properties of tissue surrounding vessels
[mus_mat,ret_mat,ori_mean,ori_std] =...
    parenchyma_optical_props(seg_wm_no_epvs,mus,ret,ori,se);
% Add to struct
parench.(sub).(loc).ves.pmus = mus_mat;
parench.(sub).(loc).ves.pret = ret_mat;
parench.(sub).(loc).ves.pori_mean = ori_mean;
...
[mus_mat,ret_mat,ori_mean,ori_std] =...
    parenchyma_optical_props(epvs,mus,ret,ori,se);
% Add to struct
parench.(sub).(loc).epvs.pmus = mus_mat;
parench.(sub).(loc).epvs.pret = ret_mat;
parench.(sub).(loc).epvs.pori_mean = ori_mean;
parench.(sub).(loc).epvs.pori_std = ori_std;
...
subjects = {'caa6', 'caa17', 'caa22', 'caa25', 'caa26'};

%%% Iterate over radius distance for measuring parenchyma


% Call the processing function
parench = parse_caa_measure_parenchyma(subjects, se);


%% Measure from edge of ves -> edge of concentric cylinder
% The purpose of this is to identify trends in the optical properties at
% increasing distances from the lumen of the vessel or EPVS. This will help
% evaluate the distance at which the optical properties are similar between
% the pathological and non-pathological.

% TODO: write a function to measure parenchyma at increasing distances
% within a disjoint cylinder


%% Function to load data
function [parench] = parse_caa_measure_parenchyma(subjects, se)
...
for i = 1:length(subjects)
    ...
    for j = 1:length(regions)
        ...
        [mus, ret, seg, seg_wm, ori, epvs] = ...
            load_local_params(eval(sub), loc);
        ...
        % Measure optical properties of tissue surrounding vessels
        [mus_mat, ret_mat, ori_std] =...
            parenchyma_optical_props(seg_wm_no_epvs, mus, ret, ori, se);

        % Add to the struct
        parench.(sub).(loc).ves.pmus = mus_mat;
        parench.(sub).(loc).ves.pret = ret_mat;
        parench.(sub).(loc).ves.pori = ori_std;
        ...
        if ~isempty(epvs)
            ...
            parench.(sub).(loc).epvs.pmus = mus_mat;
            parench.(sub).(loc).epvs.pret = ret_mat;
            parench.(sub).(loc).epvs.pori = ori_std;
        end
