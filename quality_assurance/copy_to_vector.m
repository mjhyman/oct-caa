...
function [array_mus, array_ret, array_ori, array_tissue_id, array_subid,...
         array_reg, array_stage, idx] =...
             copy_to_vector(mus, ret, sori,...
                            array_mus, array_ret, array_ori,...
                            array_tissue_id, array_subid, array_reg, ...
                            array_stage,...
                            idx, n, tiss_idx, subid, reg, stage)
...
%   subid (string): subject id (caa#)
%   reg (string): brain region (front or occip)
%   stage (int): CAA disease stage (ABC scoring)
% OUTPUTS:
...
% Update tissue ID array
array_tissue_id(idx:idx+n-1) = ones(n,1) .* tiss_idx;
% Update subject ID array
array_subid(idx:idx+n-1,1) = {subid};
% Update region array.
array_reg(idx:idx+n-1,1) = {reg};
% Update disease stage array
array_stage(idx:idx+n-1,1) = ones(n,1) .* stage;
% Iterate counter
idx = idx + n;
end